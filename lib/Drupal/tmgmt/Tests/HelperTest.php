...
class HelperTest extends TMGMTUnitTestBase {
  ...
  function testDataIemLabel() {
    ...
    $this->assertEqual(tmgmt_data_item_label($no_label, 6), 'No la…');
    $label = array(
      '#parent_label' => array(),
      '#label' => 'A label',
    );
    $this->assertEqual(tmgmt_data_item_label($label), 'A label');
    $this->assertEqual(tmgmt_data_item_label($label, 6), 'A lab…');
    $parent_label = array(
      '#parent_label' => array('Parent label', 'Sub label'),
      '#label' => 'A label',
    );
    $this->assertEqual(tmgmt_data_item_label($parent_label), 'Parent label > Sub label');
    $this->assertEqual(tmgmt_data_item_label($parent_label, 18), 'Parent… > Sub la…');
    $nested = array(
    ...
    $this->assertEqual(tmgmt_data_item_label($nested, 28), 'Parent… > Sub la… > Sub-su…');
    $long_label = array(
