...
class Job extends Entity {
  ...
  public function label($langcode = NULL) {
    ...
    $source = $languages[$this->source_language];
    if (empty($source)) {
      $source = '?';
    }
    $target = $languages[$this->target_language];
    if (empty($target)) {
      $target = '?';
    }
    return t('From @source to @target', array('@source' => $source->name, '@target' => $target->name));
  }
   ...
   *   (Optional) An array of variables to replace in the message on display.
   *
   * Use Job::cancelTranslation() to cancel a translation.
   *
   * @return \Drupal\tmgmt\Entity\Job
   *   The job entity.
   *
